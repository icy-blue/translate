...
这是附录译文。
    ...
    def test_prompts_include_planner_and_heading_rules(self):
        ...
        self.assertIn("Exclude generic wrapper headings such as `Supplementary Material`", settings.initial_prompt)
        self.assertIn("include clearly identifiable appendix or supplementary sections under them", settings.initial_prompt)
        ...
        self.assertIn("never insert `（本章仅图表）` as a placeholder for skipped local artifacts", settings.continue_prompt)
        self.assertIn("checking the full current unit from its heading to the next active unit boundary", settings.continue_prompt)
