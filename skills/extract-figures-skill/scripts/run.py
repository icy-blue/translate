...
from backend.domains.pdf_figures import extract_pdf_figures
...
def _result_error(code: str, message: str) -> dict[str, Any]:
    return {"ok": False, "error": {"code": code, "message": message}, "figures": [], "errors": []}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="extract-figures-skill runner")
    ...
    try:
        figures = extract_pdf_figures(content, preferred_direction=preferred_direction)
    except Exception as exc:
    ...
    _write_json(args.output_json, {"ok": True, "figures": encoded, "errors": []})
    return 0
