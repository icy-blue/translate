...
description: Persist a fully prepared PipelineBundle by calling backend POST /agent/pipeline/commit with x-agent-token.
metadata:
...
2. Send HTTP POST to `/agent/pipeline/commit` with `x-agent-token`.
