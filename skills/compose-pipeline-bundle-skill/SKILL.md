...
## Input
- `title` (required)
- `file_record` (required)
- `messages` (required)
- `figures/tables/tags/meta/errors` (optional)
- `conversation_id` (optional)

## Output
- `bundle` object matching backend `/agent/pipeline/commit` contract.

## Steps
1. Validate required top-level fields.
2. Normalize missing arrays to empty lists.
3. Normalize optional `meta` and `errors`.
4. Emit `bundle`.
