...
from ..domain.paper_tags import get_tag_library_payload
from ..platform.models import Conversation, PaperSemanticScholarResult, PaperTag
from .conversations import build_conversation_list_items
...
def build_filtered_conversation_statement(
    *,
    ...
    normalized_ccf_categories = ccf_categories or []
    if normalized_ccf_categories:
        ...
        if real_categories:
            ccf_conditions.append(
                Conversation.id.in_(
                    select(PaperSemanticScholarResult.conversation_id).where(PaperSemanticScholarResult.ccf_category.in_(real_categories))
                )
            )
        if "None" in normalized_ccf_categories:
            ccf_conditions.append(
                Conversation.id.in_(
                    select(PaperSemanticScholarResult.conversation_id).where(PaperSemanticScholarResult.ccf_category == "None")
                )
            )
            ccf_conditions.append(~Conversation.id.in_(semantic_ids))
        if ccf_conditions:
            statement = statement.where(or_(*ccf_conditions))
    normalized_venues = venue_filters or []
    ...
    if normalized_years:
        statement = statement.where(
            Conversation.id.in_(select(PaperSemanticScholarResult.conversation_id).where(PaperSemanticScholarResult.year.in_(normalized_years)))
        )
    return statement
...
def get_tag_usage_counts(session: Session) -> dict[str, int]:
    statement = select(PaperTag.tag_code, func.count(func.distinct(PaperTag.conversation_id))).group_by(PaperTag.tag_code)
...
def build_search_filter_payload(session: Session) -> dict:
    ...
    ccf_counts = {
        ...
        for category, count in session.exec(
            select(PaperSemanticScholarResult.ccf_category, func.count(PaperSemanticScholarResult.conversation_id)).group_by(
    ...
    venue_rows = session.exec(
        select(PaperSemanticScholarResult.venue_abbr, PaperSemanticScholarResult.venue).where(
            or_(PaperSemanticScholarResult.venue_abbr != "", PaperSemanticScholarResult.venue.is_not(None))
        )
    ).all()
    ...
    for venue_abbr, venue in venue_rows:
        ...
        entry = venue_counts.setdefault(
            value,
            {"value": value, "label": venue_abbr or venue, "full_label": venue or venue_abbr or value, "count": 0},
        )
        entry["count"] += 1
    year_counts = session.exec(
        select(PaperSemanticScholarResult.year, func.count(PaperSemanticScholarResult.conversation_id))
    ...
    return {
        ...
        "years": [{"value": str(year), "label": str(year), "count": count} for year, count in year_counts if year is not None],
    }


def search_candidates(session: Session, query: str, base_statement):
    if query:
        exact_statement = base_statement.where(Conversation.title.ilike(f"%{query}%")).order_by(Conversation.created_at.desc()).limit(5)
        exact_conversations = session.exec(exact_statement).all()
        exact_relevance_scores = [calculate_relevance(conversation.title or "", query) for conversation in exact_conversations]
    else:
        exact_statement = base_statement.order_by(Conversation.created_at.desc()).limit(10)
        exact_conversations = session.exec(exact_statement).all()
        exact_relevance_scores = [100] * len(exact_conversations)
    fuzzy_conversations = []
    ...
    if query_words:
        ...
        fuzzy_conversations = [conversation for conversation, _ in fuzzy_candidates[:5]]
        fuzzy_relevance_scores = [relevance for _, relevance in fuzzy_candidates[:5]]
    return exact_conversations, exact_relevance_scores, fuzzy_conversations, fuzzy_relevance_scores
...
def list_conversations_payload(
    *,
    ...
    limit: int,
    offset: int,
    tag_code: Optional[list[str]],
    ccf_category: Optional[list[str]],
    ...
    statement = build_filtered_conversation_statement(
        tag_codes=normalized_tag_codes,
        ccf_categories=normalized_ccf_categories,
        venue_filters=normalized_venue_filters,
        years=normalized_years,
    ).order_by(Conversation.created_at.desc())
    conversations = session.exec(statement.offset(offset).limit(limit + 1)).all()
    has_more = len(conversations) > limit
    conversations = conversations[:limit]
    return {"conversations": [item.model_dump() for item in build_conversation_list_items(session, conversations)], "has_more": has_more, "total": total}
...
def search_conversation_payload(
    ...
    *,
    ...
    normalized_venue_filters = normalize_string_filters(venue_filter)
    normalized_years = normalize_year_filters(year)
    total_conversations = count_filtered_conversations(session)
    ...
    exact_conversations, exact_scores, fuzzy_conversations, fuzzy_scores = search_candidates(session, query, base_statement)
    return {
        "exact_matches": [] if search_type == "fuzzy" else [item.model_dump() for item in build_conversation_list_items(session, exact_conversations, True, exact_scores)],
        "fuzzy_matches": [] if search_type == "exact" else [item.model_dump() for item in build_conversation_list_items(session, fuzzy_conversations, True, fuzzy_scores)],
        "total_conversations": total_conversations,
    }


def tag_library_payload(session: Session) -> dict:
...
async def get_tag_library_route(session: Session = Depends(get_db_session)):
    return tag_library_payload(session)
