...
class Settings(BaseSettings):
    ...
    initial_prompt: str = Field(
        default=(
            ...
            "- Exclude generic wrapper headings such as `Supplementary Material`, but include clearly identifiable appendix or supplementary sections under them in `appendix_units`.\n"
            ...
            "- Exclude title, authors, affiliations, emails, keywords, acknowledgements, references, bibliography, generic supplementary wrappers, standalone figure/table captions, figure legends, and table contents.\n"
    ...
    continue_prompt: str = Field(
        default=(
            ...
            "- Wrap standalone or display equations in `\\[ ... \\]` so they can be rendered by KaTeX.\n"
            "- Wrap inline mathematical expressions in `\\( ... \\)` when they contain formulas, variables, operators, superscripts, subscripts, or set notation.\n"
            ...
            "- Use `（本章仅图表）` only if, after checking the full current unit from its heading to the next active unit boundary, the entire current unit contains no non-caption running prose. If any running prose exists anywhere in the current unit, translate all such prose and do not output `（本章仅图表）`.\n"
